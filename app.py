...
# Set page config
st.set_page_config(page_title ="Saheli Beauty Mart Sales DashBoard", layout="wide")

# Helper functions
@st.cache_data
def load_data():
    data = pd.read_csv("SaheliAccount.csv")
    data['Date'] = pd.to_datetime(data['Year'].astype(str) + '-' + data['Month'].astype(str) + '-01')
    data['Date'] = pd.to_datetime(data['Date'])
    data['net_profit'] = data['gross_profit']- data['expencess']
...
def create_metric_chart(df, column, color, chart_type, height=150, time_frame='Daily', scale_to_millions = True):
    chart_data = df[[column]].copy()
    if time_frame =='Quarterly':
        chart_data.index = chart_data.index.strftime('%Y Q%q ')
    
    # Scale data to millions if specified
    if scale_to_millions:
        chart_data[column] = chart_data[column] / 1_000_000  # Convert to millions
        y_label = f"{column} (in Millions)"
    else:
        y_label = column
...
with st.sidebar:
    st.title("Saheli Beauty Mart Sales DashBoard")
    st.header("⚙️ Settings")

    max_date = df['Date'].max().date()
    dafault_start_date = max_date - timedelta(days=365) # Show a year by default
    default_end_date = max_date
    start_date = st.date_input("Start date", dafault_start_date, min_value=df['Date'].min().date(), max_value=max_date)
...
metrics = [
    ('Total Sales', 'turnover', '#29b5e8'),
    ('Gross Profit', 'gross_profit', '#FF9F36'),
    ('Expencess', 'expencess', '#D45B90'),
    ("Net Profit", "net_profit", '#7D44CF')
]
